...
void* thread_func(void* arg)
{
    ...
    // int state = pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, NULL);
    // int type = pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL);
    // int state = pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, NULL);
    int i = 1;
    while (1) {
        pthread_testcancel();
        sleep(1);
...
int main(int argc, char* argv[])
{
    ...
    pthread_t tid_child;
    int err = pthread_create(&tid_child, NULL, thread_func, "main thread");
    if (0 != err) {
