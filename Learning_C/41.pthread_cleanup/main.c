...
// 这两个函数必须成对出现，否则编译无法通过
// 注册处理程序
// void pthread_cleanup_push(void (*routine)(void*), void* arg);
// 参数routine 为调用的清理函数， arg为传给函数routine的参数

// 清除处理程序
// void pthread_cleanup_pop(int execute);
// 当execute为0时，立即执行清理程序，非0时不立即执行
// 当线程运行pthread_exit()、线程被取消、非0参数调用pthread_cleanup_pop()时，调用清理函数
...
void* thread_func(void* arg)
{
    ...
    pthread_cleanup_push(myroutine, (void*)p);
