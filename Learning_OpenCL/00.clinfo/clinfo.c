    ...
    const struct opt_out* output)
{
    ...
    if (!ret->err) {
         ...
         * which is at +0x08 */
        loc2.param.dev += (loc2.param.dev == CL_DEVICE_PREFERRED_VECTOR_WIDTH_HALF ? 0x08 : 0x30);
    ...
    const struct opt_out* output)
{
    ...
    const cl_bool get_it = (output->mode != CLINFO_HUMAN) || (loc->param.dev == CL_DEVICE_SINGLE_FP_CONFIG) || (loc->param.dev == CL_DEVICE_HALF_FP_CONFIG && dev_has_half(chk)) || (loc->param.dev == CL_DEVICE_DOUBLE_FP_CONFIG && dev_has_double(chk));
...
void device_info_fp_atomic_caps(struct device_info_ret* ret,
    const struct info_loc* loc, const struct device_info_checks* UNUSED(chk),
    const struct opt_out* output)
{
    ...
    if (!ret->err) {
        ...
        cl_uint offset = 0;
        cl_uint count = 0;
        const char* const* fpstr = (output->mode == CLINFO_HUMAN ? fp_atomic_caps_str : fp_atomic_caps_raw_str);
        set_common_separator(output);
        const char* quote = output->json ? "\"" : "";
    ...
    struct _strbuf* str, const struct opt_out* output, cl_bool these_are_offline)
{
    ...
    for (d = 0; d < ndevs; ++d) {
        ...
        if (!is_selected_device(output, p, d))
            continue;
        if (output->brief) {
            const cl_bool last_device = (d == ndevs - 1 && output->mode != CLINFO_RAW && (!output->offline || !pdata->has_amd_offline || these_are_offline));
...
struct icdl_data oclIcdProps(const struct platform_list* plist, const struct opt_out* output)
{
    ...
    if (output->mode == CLINFO_HUMAN) {
        ...
        if (icdl.reported_version && icdl.reported_version <= clinfo_highest_known_version && icdl.reported_version != icdl.detected_version) {
