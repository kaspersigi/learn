...
{
    if (output->num_selected_devices == 0)
        return CL_TRUE;

    for (cl_uint i = 0; i < output->num_selected_devices; ++i) {
        if (p == output->selected_devices[i].s[0])
            return CL_TRUE;
    }
    return CL_FALSE;
}
...
{
    if (output->num_selected_props == 0)
        return CL_TRUE;

    for (cl_uint i = 0; i < output->num_selected_props; ++i) {
        if (strstr(prop, output->selected_props[i]))
            return CL_TRUE;
    }
    return CL_FALSE;
}
