...
#define GET_STRING(str, err, cmd, param, param_str, ...)            \
    do {                                                            \
        size_t nusz;                                                \
        err = cmd(__VA_ARGS__, param, 0, NULL, &nusz);              \
        if (REPORT_ERROR(str, err, "get " param_str " size"))       \
            break;                                                  \
        realloc_strbuf(str, nusz, #param);                          \
        err = cmd(__VA_ARGS__, param, (str)->sz, (str)->buf, NULL); \
        if (REPORT_ERROR(str, err, "get " param_str))               \
            break;                                                  \
        (str)->end = nusz;                                          \
    } while (0)

#define GET_STRING_LOC(ret, loc, cmd, ...)                         \
    do {                                                           \
        size_t nusz;                                               \
